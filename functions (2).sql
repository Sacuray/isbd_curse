...
-- Вызов хранимой процедуры для аутентификации клиента
DECLARE
    is_authenticated BOOLEAN;
BEGIN
    is_authenticated := authenticate_client('client@example.com', 'password123');
...
BEGIN
    ...
    VALUES (NOW(), 'Новое', TIMESTAMP WITH TIME ZONE p_chosen_date + p_chosen_time, p_service_id, p_client_id)
...
BEGIN
    ...
    -- Этот пример предполагает наличие функции send_email(email, subject, body), которую вы должны реализовать.
    PERFORM send_email(v_client_email, 'Изменение статуса заказа', v_notification_body);
END;
$$
LANGUAGE plpgsql;

--- Изменение статуса заказа
CALL update_order_status_and_notify(123, 'В обработке');
